 ...
 *
 * @method QuirofanoQuery leftJoinReporteqx($relationAlias = null) Adds a LEFT JOIN clause to the query using the Reporteqx relation
 * @method QuirofanoQuery rightJoinReporteqx($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Reporteqx relation
 * @method QuirofanoQuery innerJoinReporteqx($relationAlias = null) Adds a INNER JOIN clause to the query using the Reporteqx relation
 *
...
abstract class BaseQuirofanoQuery extends ModelCriteria
{
    ...
    /**
     * Filter the query by a related Reporteqx object
     *
     * @param   Reporteqx|PropelObjectCollection $reporteqx  the related object to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return   QuirofanoQuery The current query, for fluid interface
     * @throws   PropelException - if the provided filter is invalid.
     */
    public function filterByReporteqx($reporteqx, $comparison = null)
    {
        if ($reporteqx instanceof Reporteqx) {
            return $this
                ->addUsingAlias(QuirofanoPeer::ID, $reporteqx->getQuirofanoId(), $comparison);
        } elseif ($reporteqx instanceof PropelObjectCollection) {
            return $this
                ->useReporteqxQuery()
                ->filterByPrimaryKeys($reporteqx->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByReporteqx() only accepts arguments of type Reporteqx or PropelCollection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the Reporteqx relation
     *
     * @param     string $relationAlias optional alias for the relation
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return QuirofanoQuery The current query, for fluid interface
     */
    public function joinReporteqx($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('Reporteqx');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'Reporteqx');
        }
    ...
    /**
     * Use the Reporteqx relation Reporteqx object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   ReporteqxQuery A secondary query class using the current class as primary query
     */
    public function useReporteqxQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinReporteqx($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Reporteqx', 'ReporteqxQuery');
    }
